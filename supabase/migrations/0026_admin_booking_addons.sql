-- 0026_admin_booking_addons.sql
-- 유선(오프라인) 예약에도 고기 추가 옵션(addons) 반영. reserve_slot과 동일한 합산 로직.
-- 파라미터 추가(p_addons)로 시그니처가 바뀌므로 기존 함수 제거 후 재생성.

drop function if exists public.admin_create_booking(text, date, smallint, text, text, int, text, text);
...
revoke all on function public.admin_create_booking(text, date, smallint, text, text, int, text, text, jsonb) from public;
grant execute on function public.admin_create_booking(text, date, smallint, text, text, int, text, text, jsonb) to service_role;
